...
async function updateAssignmentStatistics(studentId: string, assignmentId: string) {
  try {
    const sb = await getSupabase();
    ...
    // 获取该学员所有合格的作业记录，按提交时间排序
    const { data: allQualifiedSubmissions } = await sb
      .from('submissions')
      .select(`
    ...
    if (allQualifiedSubmissions && allQualifiedSubmissions.length > 0) {
      ...
      const comprehensiveStats = allQualifiedSubmissions
        .filter((sub: any) => sub.assignment_evaluation_detail) // 只包含有评估详情的
        .map((sub: any) => sub.assignment_evaluation_detail)
        .join(',');
